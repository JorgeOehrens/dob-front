...
export class Program {
  ...
  newCtorFromCode(code: Uint8Array | Buffer, name: string, symbol: string, decimals: number, admins: Array<ActorId>): TransactionBuilder<null> {
    const builder = new TransactionBuilder<null>(
      this.api,
      this.registry,
      'upload_program',
      ['New', name, symbol, decimals, admins],
      '(String, String, String, u8, Vec<[u8;32]>)',
      'String',
      code,
    );
  ...
  newCtorFromCodeId(codeId: `0x${string}`, name: string, symbol: string, decimals: number, admins: Array<ActorId>) {
    const builder = new TransactionBuilder<null>(
      this.api,
      this.registry,
      'create_program',
...
export class Vft {
  ...
  public distributeShares(shares_list: Array<[ActorId, number | string | bigint]>): TransactionBuilder<boolean> {
    if (!this._program.programId) throw new Error('Program ID is not set');
    return new TransactionBuilder<boolean>(
      this._program.api,
      this._program.registry,
      'send_message',
      ['Vft', 'DistributeShares', shares_list],
